  ...
  return (
    <div className="mb-3">
      <button onClick={() => setOpen(!open)} className="flex items-center gap-1 w-full text-left mb-1.5">
        {open ? <ChevronDown className="w-3 h-3 text-foreground-muted" /> : <ChevronRight className="w-3 h-3 text-foreground-muted" />}
        ...
        <div>
          {searchable && (
            <input
              ...
              className="w-full px-2 py-1 mb-1.5 rounded-md border border-border bg-white text-[12px] text-foreground focus:outline-none focus:border-[#b9a07a]"
            />
          )}
  ...
  return (
    <label onClick={onChange} className="flex items-center gap-2 py-1 cursor-pointer group" title={title}>
      <div
        ...
        style={{
          borderColor: checked ? '#b9703f' : '#cbc7c4',
      ...
      <span className="text-[12px] text-foreground-tertiary group-hover:text-foreground transition-colors leading-tight">{label}</span>
    </label>
  );
}
    ...
    >
      ...
      <div className="px-3.5 pt-3 pb-1 flex items-center justify-between">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <Landmark className="w-3 h-3 text-brand flex-shrink-0" />
          ...
          <div className="flex items-center gap-1">
            {isPinned && assignedPosteIds.length > 0 && (
              <div className="flex items-center gap-1">
                ...
                {assignedPosteIds.length > 3 && (
                  <span className="text-[12px] text-foreground-muted">+{assignedPosteIds.length - 3}</span>
                )}
              </div>
            )}
            <button
              onClick={(e) => { e.stopPropagation(); setShowSavePopover(!showSavePopover); }}
              className="p-1.5 -mr-1 rounded-md transition-colors hover:bg-cream flex-shrink-0"
            ...
            >
              {isPinned
                ...
                : <Bookmark className="w-3.5 h-3.5 text-border-strong group-hover:text-foreground-muted transition-colors" />
              }
            </button>
          </div>
          {showSavePopover && (
...
export default function JPSearchView({
  ...
  return (
    <div className="flex-1 flex flex-col" style={{ backgroundColor: '#F8F7F5', height: '100%', overflow: 'hidden' }}>
      ...
      <div className="flex-shrink-0 border-b border-border px-6 py-3" style={{ backgroundColor: '#F8F7F5' }}>
        <div className="flex items-center gap-3">
          <div className="flex-1 flex items-center gap-2 px-3.5 py-2 rounded-lg bg-white border border-border transition-colors"
            ...
            />
            {query && (
              <button onClick={() => setQuery('')} className="p-0.5 hover:bg-cream rounded transition-colors">
      ...
      <div className="flex-shrink-0 border-b border-border px-6" style={{ backgroundColor: '#F8F7F5' }}>
        <div className="flex items-center gap-1">
          {[
      ...
      <div className="flex-1 flex min-h-0 overflow-hidden">
        ...
        <div className="flex-shrink-0 overflow-y-auto border-r border-border" style={{ width: viewMode === 'cabinet' ? 0 : (filtersOpen ? 220 : 0), transition: 'width 0.15s ease', backgroundColor: '#fafaf9' }}>
          {filtersOpen && (
            <div className="px-4 py-3">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-1.5">
                  <SlidersHorizontal className="w-3 h-3 text-foreground-muted" />
                ...
                {activeFilterCount > 0 && (
                  <button onClick={clearFilters} className="text-[10px] text-brand hover:underline font-medium">
                    Reinitialiser
                  </button>
                )}
              </div>
              ...
              <FilterSection title="Periode">
                <div className="space-y-2">
                  <div>
                    <label htmlFor="jp-filter-date-from" className="text-[10px] text-foreground-muted uppercase font-medium" style={{ fontFamily: "'IBM Plex Mono', monospace" }}>Du</label>
                    <input id="jp-filter-date-from" type="date" value={filters.dateFrom} onChange={(e) => setFilters(prev => ({ ...prev, dateFrom: e.target.value }))}
                      className="w-full mt-1 px-2 py-1.5 rounded-md border border-border bg-white text-[12px] text-foreground focus:outline-none focus:border-[#b9a07a]" />
                  </div>
                  <div>
                    <label htmlFor="jp-filter-date-to" className="text-[10px] text-foreground-muted uppercase font-medium" style={{ fontFamily: "'IBM Plex Mono', monospace" }}>Au</label>
        ...
        <div className="flex-1 overflow-y-auto min-w-0">
          {/* Toolbar */}
          <div className="sticky top-0 z-10 px-5 py-2 border-b border-border flex items-center justify-between" style={{ backgroundColor: '#F8F7F5' }}>
            <div className="flex items-center gap-3">
              {viewMode !== 'cabinet' && (
                <button
                  onClick={() => setFiltersOpen(!filtersOpen)}
                  className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[12px] font-medium text-foreground-secondary hover:bg-cream transition-colors"
                >
            ...
            <div className="px-5 py-3" style={{ maxWidth: 1100 }}>
              {(cabinetItems || []).length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16">
                  ...
                  <p className="text-[14px] text-foreground-muted">Aucune jurisprudence de référence</p>
                  <p className="text-[12px] text-border-strong mt-1 max-w-md text-center">
            ...
            <div className="px-5 py-3">
              {results.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16">
                  ...
                  <p className="text-[14px] text-foreground-muted">Aucun resultat trouve</p>
                  <p className="text-[12px] text-border-strong mt-1">Essayez d'elargir vos criteres de recherche</p>
                </div>
              ) : (
